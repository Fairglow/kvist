...
use crate::protocol::{
    Access, AllowedSource, Cache, CachePromotion, LockfileWorkspace,
    NetworkMode, Phase, Purpose, Resources, SandboxRequest, Scratch, Toolchain,
    MAX_ARGV_ENTRIES, MAX_CACHE_BYTES, MAX_CACHE_MANIFEST_ENTRIES, MAX_ENVIRONMENT_ENTRIES,
    MAX_FILE_BYTES, MAX_FILES, MAX_GRANTS, MAX_NETWORK_SOURCES, MAX_OUTPUT_BYTES,
    MAX_PROCESSES, MAX_REQUEST_BYTES, MAX_SCRATCH_BYTES, MAX_VALUE_BYTES, MAX_WALL_TIME_MS,
...
fn validate_resources(resources: &Resources) -> Result<(), ProtocolError> {
    for (name, value, maximum) in [
        ...
        ("max_output_bytes", resources.max_output_bytes, MAX_OUTPUT_BYTES),
        ...
        ("max_scratch_bytes", resources.max_scratch_bytes, MAX_SCRATCH_BYTES),
    ] {
        if value == 0 || value > maximum {
            return Err(invalid(format!(
                "resource {name} must be nonzero and no greater than {maximum}"
            )));
        }
    }
    ...
    {
        return Err(invalid(format!(
            "resource max_cache_bytes must be nonzero and no greater than {MAX_CACHE_BYTES}"
        )));
    }
    Ok(())
}
...
fn validate_grants(request: &SandboxRequest) -> Result<(), ProtocolError> {
    ...
    let mut destinations = Vec::with_capacity(request.grants.len());
    for (index, grant) in request.grants.iter().enumerate() {
        validate_absolute(&grant.source, &format!("grant {index} source"))?;
        validate_absolute(&grant.destination, &format!("grant {index} destination"))?;
        validate_digest(&grant.identity, &format!("grant {index} identity"))?;
        if grant.access == Access::ReadWrite
            && !matches!(
                grant.purpose,
                Purpose::Authoring | Purpose::Scratch | Purpose::DependencyCache | Purpose::Lockfile
            )
        {
            return Err(invalid(format!(
                "grant {index} grants read-write access to a read-only purpose"
            )));
        }
        destinations.push(grant.destination.as_str());
    }
...
fn validate_network(request: &SandboxRequest) -> Result<(), ProtocolError> {
    ...
    match network.mode {
        NetworkMode::Deny if !network.allowed_sources.is_empty() => {
            Err(invalid("network mode `deny` must declare no allowed sources"))
        }
        NetworkMode::Deny => Ok(()),
        NetworkMode::PackageSources if request.phase != Phase::DependencyAcquisition => Err(
            invalid("network mode `package-sources` is permitted only during dependency acquisition"),
        ),
        NetworkMode::PackageSources if network.allowed_sources.is_empty() => {
            Err(invalid("network mode `package-sources` requires at least one source"))
        }
...
fn validate_acquisition(request: &SandboxRequest) -> Result<(), ProtocolError> {
    if request.network.mode != NetworkMode::PackageSources {
        return Err(invalid("dependency acquisition requires package-sources network mode"));
    }
    if request.argv.len() != 2 || request.argv[1] != "fetch" {
        return Err(invalid(
            "dependency-acquisition argv must be exactly `<cargo> fetch`",
        ));
    }
    let (cargo_identity, cargo_path) = cargo_toolchain(request)?;
    require_cargo_grant(request, cargo_path, cargo_identity)?;
    let cache = request
        .cache
        .as_ref()
    ...
    let lockfile = cache
        .lockfile
        .as_ref()
        .ok_or_else(|| invalid("dependency acquisition requires an isolated writable lockfile workspace"))?;
    let promotion = cache
        .promotion
        .as_ref()
        .ok_or_else(|| invalid("dependency acquisition requires cache-promotion metadata"))?;
    ...
    if writable.destination != cache.cargo_home {
        return Err(invalid("acquisition writable Cargo-home endpoint must equal cache cargo_home"));
    }
    validate_scratch(request, scratch)?;
    validate_lockfile(request, lockfile)?;
    ...
    validate_acquisition_promotion(request, cache, lockfile, promotion)?;
...
fn validate_verification(request: &SandboxRequest) -> Result<(), ProtocolError> {
    ...
    if request.argv.len() != 3 || request.argv[1] != "test" || request.argv[2] != "--locked" {
        return Err(invalid(
            "verification argv must be exactly `<cargo> test --locked`",
        ));
    }
    let (cargo_identity, cargo_path) = cargo_toolchain(request)?;
    require_cargo_grant(request, cargo_path, cargo_identity)?;
    let cache = request
        .cache
        .as_ref()
        .ok_or_else(|| invalid("verification requires an approved Cargo home"))?;
    let scratch = request
        .scratch
        .as_ref()
        .ok_or_else(|| invalid("verification requires writable target scratch"))?;
    let approved = cache
        .approved
        .as_ref()
        .ok_or_else(|| invalid("verification requires a read-only approved Cargo-home endpoint"))?;
    if request.resources.max_cache_bytes.is_none() {
        return Err(invalid(
            "verification against an approved Cargo home requires max_cache_bytes",
        ));
    }
    if cache.writable.is_some() || cache.lockfile.is_some() || cache.promotion.is_some() {
        return Err(invalid("verification cache may contain only the approved Cargo-home endpoint"));
    }
    validate_cache_paths(cache)?;
    require_endpoint_grant(
        request,
    ...
    if approved.destination != cache.cargo_home {
        return Err(invalid("verification approved endpoint must equal cache cargo_home"));
    }
    validate_scratch(request, scratch)?;
    require_exact_environment(
        &request.environment,
    ...
    let workspace = request
        ...
        .ok_or_else(|| invalid("verification working_directory requires a matching read-only verification grant"))?;
    let _ = workspace;
    ensure_exact_cargo_grants(
        request,
        &[Purpose::Toolchain, Purpose::DependencyCache, Purpose::Scratch, Purpose::Verification],
    )
}

fn cargo_toolchain(request: &SandboxRequest) -> Result<(&str, &str), ProtocolError> {
    let Toolchain::Cargo { identity, cargo } = &request.toolchain else {
        return Err(invalid("Cargo phases require Toolchain::Cargo"));
    };
    validate_digest(identity, "Cargo toolchain identity")?;
    validate_absolute(cargo, "Cargo toolchain path")?;
    if request.argv[0] != *cargo {
        return Err(invalid("Cargo toolchain cargo path must equal argv[0]"));
    }
    if request.identities.toolchain != *identity {
        return Err(invalid(
            "Cargo toolchain identity must equal identities.toolchain",
        ));
    }
    Ok((identity, cargo))
}

fn require_cargo_grant(
...
fn validate_scratch(request: &SandboxRequest, scratch: &Scratch) -> Result<(), ProtocolError> {
    require_endpoint_grant(
        request,
        &scratch.destination,
        &scratch.identity,
...
fn validate_lockfile(
    ...
    validate_digest(&lockfile.before_identity, "lockfile before identity")?;
    validate_digest(&lockfile.after_identity, "lockfile after identity")?;
    require_endpoint_grant(
        request,
...
fn validate_acquisition_promotion(
    request: &SandboxRequest,
    cache: &Cache,
    lockfile: &LockfileWorkspace,
    ...
    if !promotion.enabled {
        return Err(invalid("dependency acquisition requires enabled cache promotion metadata"));
    }
    ...
    if promotion.lockfile_before_identity != lockfile.before_identity
        || promotion.lockfile_after_identity != lockfile.after_identity
    {
        return Err(invalid(
            "cache promotion must bind the exact lockfile before and after identities",
        ));
    }
    let expected_sources: BTreeSet<String> = request
        .network
        .allowed_sources
        .iter()
        .map(allowed_source_identity)
        .collect();
    let actual_sources: BTreeSet<String> =
        promotion.supported_source_identities.iter().cloned().collect();
    if actual_sources.len() != promotion.supported_source_identities.len()
        || actual_sources != expected_sources
    {
        return Err(invalid(
            "cache promotion supported_source_identities must exactly bind all approved sources",
        ));
    }
    if promotion.manifest.len() > MAX_CACHE_MANIFEST_ENTRIES {
        return Err(invalid(format!(
            "cache promotion manifest exceeds the {MAX_CACHE_MANIFEST_ENTRIES}-entry limit"
        )));
    }
    let mut entries = BTreeSet::new();
    let mut aggregate = 0_u64;
    for entry in &promotion.manifest {
        validate_relative_path(&entry.path, "cache promotion manifest path")?;
        validate_digest(&entry.checksum, "cache promotion manifest checksum")?;
        if !entries.insert(&entry.path) {
            return Err(invalid("cache promotion manifest paths must be unique"));
        }
        if entry.size > request.resources.max_file_bytes {
            return Err(invalid(
                "cache promotion manifest file size exceeds max_file_bytes",
            ));
        }
        aggregate = aggregate.checked_add(entry.size).ok_or_else(|| {
            invalid("cache promotion manifest aggregate size overflowed")
        })?;
        if aggregate
            > request
                .resources
                .max_cache_bytes
                .ok_or_else(|| invalid("dependency acquisition requires max_cache_bytes"))?
        {
            return Err(invalid(
                "cache promotion manifest aggregate size exceeds max_cache_bytes",
            ));
        }
    }
    ensure_exact_cargo_grants(
...
fn ensure_exact_cargo_grants(
    ...
    if request.grants.len() != expected_purposes.len()
        ...
        || expected_purposes
            .iter()
            .any(|purpose| request.grants.iter().filter(|grant| grant.purpose == *purpose).count() != 1)
    {
        return Err(invalid(
...
fn require_endpoint_grant(
    ...
    validate_absolute(destination, &format!("{label} destination"))?;
    ...
    if count != 1 {
        return Err(invalid(format!(
            "{label} must correspond exactly to one matching {purpose:?} grant"
        )));
    }
    Ok(())
}
...
fn require_exact_environment(
    ...
    if environment.len() != required_names.len()
        || environment.keys().any(|name| !required_names.contains(name.as_str()))
    {
        return Err(invalid(
    ...
    for (name, expected) in required {
        let actual = environment.get(*name).ok_or_else(|| {
            invalid(format!("Cargo phase environment requires `{name}`"))
        })?;
        if *name == "PATH" {
    ...
    if !environment.contains_key("PATH") {
        return Err(invalid(
            "Cargo phase environment requires a single explicit canonical PATH",
        ));
    }
    validate_path_environment(
        environment
            .get("PATH")
            .ok_or_else(|| invalid("Cargo phase environment requires PATH"))?,
    )
}
...
fn allowed_source_identity(source: &AllowedSource) -> String {
    match source {
        AllowedSource::CargoRegistry {
            name,
            index_origin,
            download_origin,
            ..
        } => registry_identity(name, index_origin, download_origin),
        AllowedSource::CargoGit {
            repository,
            revision,
            ..
        } => git_identity(repository, revision),
    }
}
...
pub fn registry_identity(name: &str, index_origin: &str, download_origin: &str) -> String {
    source_identity(
        ...
        &[name.as_bytes(), index_origin.as_bytes(), download_origin.as_bytes()],
    )
}
...
fn validate_absolute(value: &str, label: &str) -> Result<(), ProtocolError> {
    ...
    if !value.starts_with('/') || value.contains('\0') {
        return Err(invalid(format!("{label} must be an absolute path without NUL")));
    }
...
fn validate_relative_path(value: &str, label: &str) -> Result<(), ProtocolError> {
    ...
    if value.is_empty() || value.starts_with('/') || value.contains('\0') {
        return Err(invalid(format!("{label} must be a nonempty relative path")));
    }

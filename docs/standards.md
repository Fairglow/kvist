...
| Reproducible, offline builds   | Tailored local practice                                  | Dependencies are vendored from the exact locked versions on the host, recorded in a versioned manifest, and resolved offline inside the network-denied sandbox; network toolchain operations run as a bounded, approved host phase (ADR 0011). |

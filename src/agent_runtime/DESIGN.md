...
| `supervisor` | Process groups, streams, idle/loop/wall/output limits, retry, cancellation |
...
group, reads stdout/stderr through bounded nonblocking channels, forwards
output, and tracks idle, wall, output, loop, and cancellation conditions.
Terminal cleanup ends and reaps the process group before the result returns.
Only idle or loop outcomes may create a new attempt with prior-failure context.
...
selection rejects conflicts before any provider starts.
...
caller. Retry notices include attempt number, prior failure class, and
uncertain-side-effect warning.
